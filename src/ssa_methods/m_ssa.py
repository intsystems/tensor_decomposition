...
class m_SSA(SSA_Base):
    ...
    def dispose_factors(self, indices: tuple):
        """remove factors with given indices (for all signals)

        :param tuple indices: factors to remove
        """
        # output relative norm of disposed factors
        print(f'Relative norm lost = {linalg.norm(self.weights[indices], ord=2) / linalg.norm(self.weights, ord=2)}')
