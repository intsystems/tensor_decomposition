...
class t_SSA(SSA_Base):
    ...
    def dispose_factors(self, indices: list, signal_num: int):
        ...
        # output relative norm of disposed factors
        print(f'Relative norm lost = {linalg.norm(self.weights[signal_num][indices], ord=2) / linalg.norm(self.weights[signal_num], ord=2)} for signal {signal_num + 1}')

        # reset supportive vectors to recalculate it futher
        self._pred_vecs[signal_num] = []

    
    def decompose_tt(self, cpd_rank: int=None):
